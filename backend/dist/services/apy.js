...
function updateCachedApys(newApys) {
    try {
 ...
 */
export async function updateApys() {
    const current = getCachedApys();
    const nextApys = {};
    for (const [key, addr] of Object.entries(VAULTS)) {
        try {
            ...
            nextApys[key] = Math.max(2.0, parseFloat((base * multiplier + fluctuation).toFixed(2)));
        }
        catch (err) {
            // Fallback fluctuation if blockchain connection fails
            const base = current[key];
            const fluctuation = (Math.random() - 0.5) * 0.15;
            nextApys[key] = Math.max(2.0, parseFloat((base + fluctuation).toFixed(2)));
        }
    }

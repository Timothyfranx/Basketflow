...
export interface QuoteRequest {
  inputTokenSymbol: keyof typeof TOKENS;
  amountIn: string; // e.g. "1000"
  targetTokenASymbol: keyof typeof TOKENS;
  targetTokenBSymbol: keyof typeof TOKENS;
...
export interface QuoteResponse {
  amountAOut: string;
  amountBOut: string;
  minAmountAOut: string;
  minAmountBOut: string;
  pathA: string[];
  pathB: string[];
  isMock: boolean;
}
  ...
  try {
    ...
    if (inputAddr !== addrA) {
      const amountsOutA = await client.readContract({
        ...
        args: [amountAInBigInt, pathA as `0x${string}`[]],
      });
    ...
    if (inputAddr !== addrB) {
      const amountsOutB = await client.readContract({
        ...
        abi: ROUTER_ABI,
        functionName: "getAmountsOut",
        args: [amountBInBigInt, pathB as `0x${string}`[]],
      });
